...
def reg():
    ...
    jalan('  EASYPAISA ACCOUNT NO (+8801982876336) ')
    jalan('  EASYPAISA ACCOUNT NAME (MR RAFU) ')
    ...
    r = requests.get('https://github.com/Rafu0878/rafuapproval.txt.git').text
    if to in r:
...
def main():
    ...
    jalan('  AXI MAIN MENU')
    print ' \x1b[1;97m    [RAFU] MAIN MENU'
    print '\x1b[1;97m-----------------------------------------------------'
            ...
            while True:
                ...
                def __yan__(ysc = (None, None)):
                    ...
                    if cin == '':
                        ...
                        print '\n [%s+%s] RAFU OK file -> results/OK-%s-%s-%s.txt' % (O, N, ha, op, ta)
                        print ' [%s+%s] RAFU CP file-> results/CP-%s-%s-%s.txt' % (O, N, ha, op, ta)
