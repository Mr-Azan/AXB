...
banner ="""
  ...
  YP   YP d88888P YP   YP VP   V8P 
=======================================
  ➣   Author      :   A Z A N 
  ➣   YouTube :       ITNA FALTU TIME NI HAIN
...
def login_lagi334():
	...
	warna = random.choice([
 P, M, H, K, B, U, O, N])
	yu = ' Ketik [green]Get[/green] If U Want To Know How To Get cookie'
	yi = nel(yu,style='')
	sol().print(yi)
	cookie=input(f'  [{h}•{x}] Input Cookie :{warna} ')
	...
	try:
		...
		print(f'  {x}[{h}•{x}]{h} LOGIN Successful.........Run the Oder Again!!!!{x} ');time.sleep(1)
		exit()
	except Exception as e:
		os.system("rm -f .token.txt")
		os.system("rm -f .cok.txt")
		print(f'  %s[%sx%s]%s LOGIN Failed.....Input fresh cookies !!%s'%(x,k,x,m,x))
		exit()
...
def result():
	...
	if kz in ['1','01']:
		...
		if len(vin)==0:
			...
			gerr2 = '# CHECK RESULT CRACK'
			sol().print(mark(gerr2, style='red'))
			geeh = input(N+'['+M+'➣'+N+'] MENU : ')
			try:geh = lol[geeh]
			except KeyError:
				ric = '# Chooose The Correct  MENU'
