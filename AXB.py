...
def uaku():
	try:
		...
		a=requests.get('https://github.com/IKFAR-IFC/IFCV1/blob/main/ua2.txt').text
		ua=open('.ua2.txt','w')
...
##     ## ######## ##     ## ##    ## 
    ...
    {m}{k}{h}{sir} AUTHOR : A Z A N{x}{m}{k}{h}{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def login_lagi334():
	try:
		...
		print(f'  {x}[{h}•{x}]{h} TOLONG BANTU LIKE NGAB!!!!{x} ')
		os .system ('xdg-open https://www.facebook.com/100057411861234/posts/pfbid02ZWXYSG13G82tUcyhVU4ZwDeNHy4ssSCGZVZ6d7xiDfzmSjjxcodBGQmJzUqXuoGcl/?app=fbl');#line:52
		print(f'  {x}[{h}•{x}]{h} LOGIN BERHASIL.........Tunggu Sabar Dek!!!!{x} ');time.sleep(1)
		os .system ('python IFCV1.py')
	except Exception as e:
...
def menu(my_name,my_id):
	...
	cetak(nel('\tSelamat Datang [green]%s[white] Bego'%(my_name)))
	print(f'│ ╰─[{H}MENU{N}]──────────────────────────────────[\033[41m\033[1;37m IKFAR-IFC \x1b[0m\x1b[1;97m{N}]╯')
	print(f'{N}├───[{asu}+{N}] ID KAMU : '+str(my_id))
	print(f'{N}├───[{asu}+{N}] IP KAMU  : {ip}')
	print(f'{N}├───[{asu}✓{N}]{H} Status   {N}: \033[41m\033[1;37m PERTALIT \x1b[0m\x1b[1;97m{N}')
...
def dump_massal():
	...
	try:
		...
		print(f'{N}╰─> {x}[{h}•{x}]{h} JOIN GRUP WhatsApp DULU BIAR GA KETINGGALAN UPDATE!!!!{x} ')
		os .system ('xdg-open https://chat.whatsapp.com/JGUaVNc1DFO8rYRNAcXmJU');#line:52
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
...
def token_cookie():
	...
	try:
		...
		for x in range(limit):
			...
			if 'id' in post:
				print(f" {n}. IKFAR-IFC BERHASIL MEMBAGIKAN POST:  {data['id']}")
			else:
...
def setting():
	...
	print(f'{N}├───[{H}2{N}] Akun New {H}[RECOMMEND]{N}')
	...
	if hu in ['1','01']:
		...
		exit()
	print(f'{N}├───────────────────────────────────────────────────────')
	print(f'{N}├───[{H}1{N}] Mobile {H}[SANGAT RECOMMENDED]{N}')
	print(f'{N}├───[{H}2{N}] Mbasic ')
	print(f'{N}├───[{H}3{N}] Touch  ')
	print(f'{N}├───[{H}4{N}] Mtouch ')
	print(f'├──╭[{h}PILIH METODE{N}]────────────────────────────────────────────')
	hc = input(f'│  ╰─➣{h} ')
	if hc in ['1','01']:
	...
	#else:
		#taplikasi.append('no')
	pwplus=input(f'{N}├───[{H}+{N}] Tambahkan Password Manual ({k}Y{N}/{h}T{N}) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Masukkan Katasandi Tambahan Minimal 6 Karakter\n[[cyan]•[white]] Contoh :[green] Katasandi,rahasia,123456[white] '))
...
def passwrd():
	...
	jalan(f'{N}│ │ [{k}•{x}] Hasil {k}CP{x} Tersimpan Di : {k}%s{x}  {N}│'%(cpc))
	jalan(f'{N}│ ╰────────[{k}✨ON/OF MODE PESAWAT SETIAP 500 ID✨{N}]─────────╯') #%(okc,cpc))
	jalan(f"│")
...
def crack(idf,pwv,nmf):
	...
	fff = '%'
	sys.stdout.write('\r└\x1b[1;90m[%s\x1b[1;93mAXB\x1b[1;90m]-[\x1b[1;91m%s\x1b[1;90m/\33[m%s\x1b[1;90m]-[\x1b[1;92mOK:%s\x1b[1;90m]-[\33[93mCP:%s\x1b[93m]-[\x1b[1;94m%s%s\x1b[1;90m]%s ✓'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x));sys.stdout.flush()
