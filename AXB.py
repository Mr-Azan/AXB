...
\t|_______/[___]   \'-;__/[_____][_____]'.__.'[___]    	
			{m}•{k}•{h}•{sir} Author : Z O X {x}{m}•{k}•{h}•{x}''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
def setting():
	...
	if gua in ["1","01"]:
		...
		method.append('mobile')
	print('|')
	pwplus=input('├──> Chosse Password Manual ( Y/t ) : ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[cyan]•[white]] Input Six minimun password \n[[cyan]•[white]] Contoh :[green] mantan,selebew,bagong[white] '))
		pwku=input('├──> InputPassword Tambahan : ')
		pwkuh=pwku.split(',')
		for xpw in pwkuh:
			pwnya.append(xpw)
	else:
