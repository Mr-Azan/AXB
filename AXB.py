...
╚══███╔╝██╔═══██╗╚██╗██╔╝
 ...
 ███╔╝  ██║   ██║ ██╔██╗ 
███████╗╚██████╔╝██╔╝ ██╗
╚══════╝ ╚═════╝ ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║Z O X FACEBOOK METHOD║▌║▌║█│▌{N} """)
    print("╔════════════════════════════════════════╗")
    print(f"║{P}[{H}•{P}] Author    : Z O X                  {N}║")
    print(f"║{P}[{H}•{P}] Whatsapp  : +923165456679           {N}║")
    print(f"║{P}[{H}•{P}] Github    : github.com/AXB/ZOX{N}  ║")
...
# METODE SANDI MANUAL
    def plerr(self,id):
        ...
        if ___yayanganteng___ in ('raraY', 'raray'):
            ...
            print(' %s[%s!%s] Use %sKOMMA%s for separator\n [%s•%s] Example : %skhan1122%s,%skhankhan%s,%skhan786'%(N,M,N,H,N,H,N,H,N,H,N,H))
            while True:
...
# PROSES CRACK METODE 3 in 1
    ...
    def kombinasi_pw(self,url):
        ...
        print(' %s[%s3%s] name,name123,name1234,name12345,%s+Sandi%s'%(N,H,N,H,N))
