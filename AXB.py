# DECOMPILED BY RANA NADEEM RAJPUT
# DONT MESS WITH MW
...
-----------------------------------------------
 [1] Login
 [2] Grap links from file
 [3] Remove duplicate links from file
 [4] Join rsa group only for subscribers
 [5] How to use this tool videos
-----------------------------------------------
 select an option:
--------------------------------------------------"""
oks=[]
...
def auto():
    ...
    for fir in range(limit_user):
        ...
        try:
            ...
            fr = requests.get('https://graph.facebook.com/'+udit+'/friends?access_token='+tfile).text
            qfr = json.loads(fr)
    ...
    for ids in fileidopen:
        try:
            tfile = open('access_token.txt','r').read()
            rg = requests.get('https://graph.facebook.com/'+ids+'/friends?access_token='+tfile).text
            rgq = json.loads(rg)
