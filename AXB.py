...
def result():
	print(f'[1]. CHECK OK IDZ ')
	print(f'[2]. CHECK CP IDZ ')
	print('[3]. Kembali	')
	kz = input(f'\n<\> SELECT : ')
	if kz in ['2']:
		...
		if len(vin)==0:
			...
			for cpku in range(len(lin)):
				cpkuni=lin[nocp].split('|')
				print('')
				print(f'>> {cpkuni[0]}|{cpuntil[1]}|{cpuntil[2]}')
				nocp +=1
			print('')
			input(f'[Klik Enter]')
			back()
...
def alok():
    try:
        ...
        if pil =="":
            ...
            print('%s══════════════════════════════════════════'%(N))
            print(' %s[%s!%s] Input %sUsername & Password%s Untuk Masuk ke tools!'%(N,M,N,H,N))
        else:
