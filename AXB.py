...
╚══════╝ ╚═════╝ ╚═╝  ╚═╝\n{N} ▌│█║▌║▌║Z O X FACEBOOK METHOD║▌║▌║█│▌{N} """)
    ...
    print(f"║{P}[{H}•{P}] Network   : {H}ALL{N}, {H} NETWORK {N}, {H}3{N}         ║")
	...
	try:
        ...
        if pil =="":  
         jalan(f" {N}[{M}×{N}] Sorry, it is wrong...!");time.sleep(1);cek_pw()
            ...
            os.system('xdg-open https://wa.me/3165456679?text=Hallo+izin+menggunakan+SC+ini');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
