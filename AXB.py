...
# code by Azan XD
# my facebook ( https://www.facebook.com/Azan )
...
def logo():
  ...
  ╚═╝  ╚═╝╚═╝  ╚═╝╚═════╝\n{N} ▌│█║▌║▌║META FACEBOOK METHOD║▌║▌║█│▌{N} """)
    ...
    print(f"║{P}[{H}•{P}] Github    : github.com/Mr-AZAN/Z O X{N}  ║")
    ...
    print(f"║{P}[{H}•{P}] SC Version: V-5.5 ({K}Z O X{P})            {N}║")
    print("╚════════════════════════════════════════╝")
#CRACK SELESAI
def hasil(ok,cp):
...
def cek_pw():
    try:
        ...
        if pil =="":
            ...
            os.system('xdg-open https://www.facebook.com/profile.php?id=100003561369879');time.sleep(2);cek_pw()
        elif pil in["1","01"]:
            print('%s══════════════════════════════════════════'%(N))
...
def yayanxd():
    os.system('clear')
    try:
    	___kontol___ = input('[|] Put Cookies : ')
...
def moch_yayan(my_name,my_id):
    ...
    print(' [%s08%s] Check Crack Results (%sON%s)'%(H,N,H,N));time.sleep(0.03)
    print(' [%s09%s] SC update info'%(H,N));time.sleep(0.03)
...
###### CEK HASIL CRACK #####
        ...
        for memek in total:
            kontol = memek.replace("\n","")
            titid  = kontol.replace("","\x1b[1;92m").replace("", "\x1b[1;93m")
...
def log_hasil(user, pasw):
    ...
    if "c_user" in session.cookies.get_dict():
        if "Akun Anda Dikunci" in urlPost.text:
            ...
            print(f"\r {N}[{H}✓{N}] {H}Account unlocked{N}");time.sleep(0.03)
...
# PROSES CRACK METODE 3 in 1
    ...
    def kombinasi_pw(self,url):
        ...
        print(' %s[%s1%s] name,name123,name12345'%(N,H,N))
        print(' %s[%s2%s] name,name123,name1234,nama12345'%(N,H,N))
        print(' %s[%s3%s] name,name123,name1234,nama12345,%s+Password%s'%(N,H,N,H,N))
